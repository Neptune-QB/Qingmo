...
6. 必须给出 confidence，范围 0 到 1，按以下标准打分：
   - 0.9-1.0: 台词明确表达情绪 + 画面中人脸表情清晰可辨 + 剧情冲突强烈
   - 0.75-0.89: 台词暗示情绪但不够直白，或画面描述模糊
   - 0.6-0.74: 仅凭台词或画面单一维度判断，另一维度缺失或证据不足
   不要全部给同一个分数，不同高光点的 confidence 必须有区分度。
7. 必须给出 evidence。
